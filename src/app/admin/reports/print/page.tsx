...
function ReportPage({
  ...
  return (
    <div className="report-page" style={{ pageBreakAfter: isLast ? 'avoid' : 'always', breakAfter: isLast ? 'avoid' : 'page' }}>
      {/* Page header */}
      <div className="page-header">
        ...
        <div className="page-header-right">
          ...
          <div className="header-submitted">
            Submitted by {report.submitted_by_name || report.submitted_by_email}
          </div>
        </div>
      </div>
      ...
      {/* Attraction name */}
      <h1 className="attraction-title">{attraction?.name || 'Unknown Attraction'}</h1>
      ...
      <div className="stats-row">
        ...
        <div className="stat-box">
          ...
          <div className="stat-value" style={{ color: delays.length > 0 ? '#c0392b' : undefined }}>
            {delays.length > 0
              ? formatMinutes(delays.reduce((s, d) => s + (d.duration_minutes || 0), 0))
              : '—'}
          </div>
        </div>
      </div>
              ...
              {delays.map((d, i) => (
                <tr key={i}>
                  <td style={{ whiteSpace: 'nowrap' }}>
                    {formatTs(d.started_at)}
                    {d.resolved_at ? ` – ${formatTs(d.resolved_at)}` : ' (unresolved)'}
                  </td>
                  <td>{d.reason || '—'}</td>
      ...
      <div className="page-footer">
        ...
        <span>{attraction?.name} · {formatDate(report.report_date)}</span>
      </div>
    </div>
  );
}
